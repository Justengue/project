...
VertexItem::VertexItem(std::string const &line)
{
  ...
  std::istringstream{line} >> vid_ >> latitude_ >> longitude_;
  setFlags(ItemIsSelectable);
  setAcceptHoverEvents(true);
  setPos(latitude_, longitude_ );
...
void VertexItem::paint(QPainter *painter, const QStyleOptionGraphicsItem *option, QWidget */*widget*/)
{
    ...
    if (lod >= 2) {
      ...
      painter->drawText(10, 20, QString("X=%1").arg(latitude_));
      painter->drawText(10, 30, QString("Y=%1").arg(longitude_));
    } else if (lod >= 0.6) {
      QFont font("Times", 5);
      font.setStyleStrategy(QFont::ForceOutline);
      painter->setFont(font);
