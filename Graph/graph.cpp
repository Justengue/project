...
Edge Graph::getEdge(Vertex v1, Vertex v2){
    ...
    for(Edge e : edgeList){ 
        if(e.getV1Id() == v1.getId() && e.getV2Id() == v2.getId()) return e;
    }
...
void Graph::neighbors(Vertex v, vector<Vertex> &list){
    ...
    for(Edge e : edgeList){
        ...
        if(e.getV1Id() == v.getId()){
            auto it = vertList.find(e.getV2Id());
            list.push_back(it->second);
        }
    }
}

void Graph::bfs(uint32_t vstart, uint32_t vstop){
    ...
    while(active_queue.size() != 0){
        ...
        for(Vertex vnext : adjacency_list){
            ...
            dist = getEdge(vcurrent, vnext).getDist();
...
void Graph::dijkstra(uint32_t vstart, uint32_t vstop){
...
void Graph::astar(uint32_t vstart, uint32_t vstop){
